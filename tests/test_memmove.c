...
void run_tests() {
    ...
    memmove(dst1, src1, strlen(src1) + 1); // Standard memmove
    ft_memmove(dst1_ft, src1, strlen(src1) + 1); // Custom ft_memmove
    compare_results(dst1, dst1_ft, "Basic non-overlapping test");

    // 2. Overlapping forward copy
    char str2[] = "abcdefgh";
    char str2_ft[] = "abcdefgh";
    memmove(str2 + 3, str2, 5); // Standard memmove
    ft_memmove(str2_ft + 3, str2_ft, 5); // Custom ft_memmove
    compare_results(str2, str2_ft, "Overlapping forward copy");

    // 3. Overlapping backward copy
    char str3[] = "abcdefgh";
    char str3_ft[] = "abcdefgh";
    memmove(str3, str3 + 3, 5); // Standard memmove
    ft_memmove(str3_ft, str3_ft + 3, 5); // Custom ft_memmove
    compare_results(str3, str3_ft, "Overlapping backward copy");

    // 4. Copying to the same memory
    char str4[] = "Hello, World!";
    char str4_ft[] = "Hello, World!";
    memmove(str4, str4, strlen(str4) + 1); // Standard memmove
    ft_memmove(str4_ft, str4_ft, strlen(str4_ft) + 1); // Custom ft_memmove
    ...
    char dst5[20] = {0};
    char dst5_ft[20] = {0};
    memmove(dst5, src5, 0); // Standard memmove
    ft_memmove(dst5_ft, src5, 0); // Custom ft_memmove
    compare_results(dst5, dst5_ft, "Zero length (nothing to copy)");

    // 6. Partial overlapping memory
    char str6[] = "123456789";
    char str6_ft[] = "123456789";
    memmove(str6 + 4, str6 + 2, 5); // Standard memmove
    ft_memmove(str6_ft + 4, str6_ft + 2, 5); // Custom ft_memmove
    ...
    int dst7[5] = {0};
    int dst7_ft[5] = {0};
    memmove(dst7, src7, sizeof(int) * 5); // Standard memmove
    ft_memmove(dst7_ft, src7, sizeof(int) * 5); // Custom ft_memmove
